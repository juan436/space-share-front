...
export class ApiAdminRepository implements AdminRepository {
  ...
  async getStats(): Promise<AdminStats> {
    const res = await this.httpClient.get<AdminStats>("/admin/stats");
    return res.data;
  }

  async getUsers(): Promise<AdminUser[]> {
    const res = await this.httpClient.get<AdminUserDto[]>("/admin/users");
    return res.data.map(AdminMapper.userToDomain);
  }

  async getSpaces(): Promise<AdminSpace[]> {
    const res = await this.httpClient.get<AdminSpaceDto[]>("/admin/spaces");
